...
<div class="flex items-center">
    ...
    @if (!str($status)->startsWith('Proxy') && !str($status)->contains('('))
        @if (str($status)->contains('unhealthy'))
            <x-helper popupclass="right-0" helper="Unhealthy state. <span class='dark:text-warning text-coollabs'>This doesn't mean that the resource is malfunctioning.</span><br><br>- If the resource is accessible, it indicates that no health check is configured - it is not mandatory.<br>- If the resource is not accessible (returning 404 or 503), it may indicate that a health check is needed and has not passed. <span class='dark:text-warning text-coollabs'>Your action is required.</span><br><br>More details in the <a href='https://coolify.io/docs/knowledge-base/traefik/healthcheck/' class='underline dark:text-warning text-coollabs' target='_blank'>documentation</a>."  >
                <x-slot:icon>
